...
public class BinarySearch {
    ...
    // then the first and last not be a peak element so we take from st = 1 and ed =
    // arr.length - 1
    ...
    //most optimized way of single array
    static int singleElementOptimal(int[] arr) {
        int st = 0;
        int ed = arr.length - 1;
        if (arr.length == 1) {
            return arr[0];
        ...
        while (st <= ed) {
            ...
            if (md == 0 && arr[md] != arr[md+1]) return arr[md];
            if (md == arr.length-1 && arr[md] != arr[md-1]) return arr[md];
